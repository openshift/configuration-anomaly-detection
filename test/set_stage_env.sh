...
for v in $(vault kv get  -format=json osd-sre/configuration-anomaly-detection/grafana/stg | jq -r ".data.data|to_entries|map(\"\(.key)=\(.value|tostring)\")|.[]"); do export $v; done
