...
func run(cmd *cobra.Command, args []string) error {
	...
	cssJumprole, ok := os.LookupEnv("CAD_CSS_JUMPROLE")
	if !ok {
		return fmt.Errorf("CAD_CSS_JUMPROLE is missing")
	}

	supportRole, ok := os.LookupEnv("CAD_SUPPORT_JUMPROLE")
	if !ok {
		return fmt.Errorf("CAD_SUPPORT_JUMPROLE is missing")
	}
