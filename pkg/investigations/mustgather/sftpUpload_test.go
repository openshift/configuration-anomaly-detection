...
func TestCopyWithContext(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: strings.Repeat("A", 100*1024), // 100KB - requires multiple 32KB chunks
			setupCtx: func() (context.Context, context.CancelFunc) {
				return context.WithTimeout(context.Background(), 5*time.Second)
			},
			setupWriter: func() io.Writer {
		...
		},
		{
			...
			input: strings.Repeat("D", 64*1024), // 64KB to span multiple chunks
			setupCtx: func() (context.Context, context.CancelFunc) {
				return context.WithTimeout(context.Background(), 5*time.Second)
			},
			setupWriter: func() io.Writer {
				return &errorWriter{
					errorAt: 32 * 1024, // Error exactly after first chunk (32KB)
		...
		},
		{
			...
			input: strings.Repeat("C", 64*1024), // 64KB to ensure short write is detected
			setupCtx: func() (context.Context, context.CancelFunc) {
				return context.WithTimeout(context.Background(), 5*time.Second)
			},
			setupWriter: func() io.Writer {
				return &errorWriter{
					shortAt: 32 * 1024, // Short write after 32KB
				}
			},
			setupReader: func(data []byte) io.Reader {
				return bytes.NewReader(data)
