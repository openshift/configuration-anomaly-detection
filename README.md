...
2. Clone the [backplane-api](https://github.com/openshift/backplane-api) repository to a local directory and fetch `ocm.json` from a staging cluster into its `./configs` dir.
   ...
   See the [local backplane CAD testing SOP](https://github.com/openshift/ops-sop/blob/master/v4/howto/backplane-local-cad-testing.md) for instructions on obtaining `ocm.json`.
